# Clustering factor and TABLE ACCESS BY INDEX ROWID cost

## Symptom
A non-covering seller/date index returns many rowids scattered across table blocks. Index selectivity looks good but physical/table block visits dominate.
...
## Evidence to capture
Primary signals: clustering_factor vs num_blocks, table access by rowid batched, consistent gets and index/table layout.
