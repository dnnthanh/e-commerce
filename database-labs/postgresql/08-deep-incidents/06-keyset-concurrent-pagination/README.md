# Concurrent pagination without duplicate/missing rows

## Symptom
Support users page through a mutable audit/order stream while new rows arrive. OFFSET is slow deep in the result set and unstable when created_at ties.
...
## Evidence to capture
Primary signals: rows scanned for OFFSET, stable composite ordering, duplicate/missing detection under concurrent inserts and latency by page depth.
