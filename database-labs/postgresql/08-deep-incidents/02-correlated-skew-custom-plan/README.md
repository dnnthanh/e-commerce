# Correlated seller/status skew and generic plan regression

## Symptom
A prepared order-history query is fast for normal sellers but slow for the marketplace mega-seller because seller_id and status are strongly correlated.
...
## Evidence to capture
Primary signals: generic_plans vs custom_plans, pg_stats/extended statistics, estimate error on seller_id+status and plan flip between Bitmap Heap Scan and Seq Scan.
