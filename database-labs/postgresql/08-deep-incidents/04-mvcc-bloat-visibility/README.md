# Long transaction, MVCC bloat and visibility-map loss

## Symptom
A reporting transaction stays open for hours while workers update inventory/outbox rows. Autovacuum runs but table/index size grows and index-only scans degrade into heap fetches.
...
## Evidence to capture
Primary signals: backend_xmin, n_dead_tup, last_autovacuum, visibility map effect, heap fetches, relpages/index size and oldest transaction age.
