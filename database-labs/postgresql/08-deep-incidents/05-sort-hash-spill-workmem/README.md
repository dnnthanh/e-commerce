# Sort/hash spill, work_mem and parallel plan trade-offs

## Symptom
A settlement aggregation groups millions of lines and sorts by seller/date. Production plan spills to temp files; simply increasing work_mem risks multiplying memory by parallel workers and plan nodes.
...
3. Capture EXPLAIN (ANALYZE, BUFFERS, WAL, SETTINGS, VERBOSE).
...
## Evidence to capture
Primary signals: Sort Method external merge, Disk, Hash Batches, temp read/write buffers, workers planned/launched and memory concurrency budget.

Also correlate with pg_stat_statements, pg_stat_activity, pg_locks, pg_stat_user_tables, pg_stat_user_indexes. Record database version and session-level settings because optimizer/memory behavior is version and configuration sensitive.
