# High-concurrency outbox claiming with SKIP LOCKED

## Symptom
Thirty workers claim pending events. Throughput collapses when old processed rows dominate the table and several workers target the same hot index pages.
...
## Evidence to capture
Primary signals: Lock wait events, rows claimed per transaction, index-only eligibility, dead tuples, heap fetches and WAL per claimed event.
