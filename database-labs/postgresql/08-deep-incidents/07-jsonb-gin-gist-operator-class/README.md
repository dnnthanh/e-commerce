# JSONB GIN operator classes and GiST range overlap

## Symptom
Catalog attributes are JSONB while promotions use validity ranges. One broad GIN index becomes huge and range overlap scans still seq-scan.
...
## Evidence to capture
Primary signals: index size, pending list, operator compatibility jsonb_ops vs jsonb_path_ops, lossy bitmap rechecks and GiST overlap selectivity.
