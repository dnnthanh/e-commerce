# Partition pruning, BRIN/B-tree choice and index write budget

## Symptom
A time-ordered ledger grows past hundreds of millions of rows. Teams add indexes until ingestion slows; some queries still scan many partitions because predicates wrap the partition key.
...
## Evidence to capture
Primary signals: Subplans Removed, partition pruning time, BRIN correlation, B-tree size, WAL bytes per insert and redundant-index usage.
