# Parameter sniffing, skew and Parameter Sensitive Plan optimization

## Symptom
A stored order search compiles for a tiny customer then reuses a Nested Loops plan for a whale customer with millions of rows.
...
3. Capture Actual Execution Plan + SET STATISTICS IO, TIME ON.
...
## Evidence to capture
Primary signals: actual vs estimated rows, compiled parameter, Query Store plan variants, logical reads and PSP eligibility.

Also correlate with Query Store, sys.dm_exec_query_stats, sys.dm_exec_requests, sys.dm_tran_locks, Extended Events. Record database version and session-level settings because optimizer/memory behavior is version and configuration sensitive.
