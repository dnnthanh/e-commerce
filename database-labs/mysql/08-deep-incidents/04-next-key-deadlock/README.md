# Next-key locks, gap locks and deadlock ordering

## Symptom
Concurrent reservation transactions update inventory ranges in different SKU order. REPEATABLE READ adds next-key locking and deadlocks appear only under load.
...
## Evidence to capture
Primary signals: SHOW ENGINE INNODB STATUS deadlock graph, performance_schema data_locks, trx isolation and lock acquisition order.
