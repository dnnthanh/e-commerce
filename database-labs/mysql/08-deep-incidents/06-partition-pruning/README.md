# Range partition pruning and local maintenance

## Symptom
Monthly event tables are partitioned but queries apply DATE(created_at) and scan every partition. Retention DELETE also creates undo/redo pressure.
...
## Evidence to capture
Primary signals: partitions field in EXPLAIN, predicate sargability, exchange/drop partition maintenance and local index cost.
