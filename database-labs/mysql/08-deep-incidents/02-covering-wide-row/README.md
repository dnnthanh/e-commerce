# Covering index versus wide/LOB row fetch

## Symptom
Selecting only account number is fast while SELECT * is slow because JSON/TEXT payloads force clustered-row lookups and buffer-pool churn.
...
## Evidence to capture
Primary signals: secondary vs clustered lookups, rows_examined, buffer pool reads, table row format and LOB projection cost.
