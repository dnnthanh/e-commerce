# Composite indexes, leftmost prefix and hot-seller skew

## Symptom
Order search filters seller, status and date, but optional predicates cause the optimizer to use a weak prefix or scan too many secondary-index entries for a hot seller.
...
3. Capture EXPLAIN ANALYZE plus EXPLAIN FORMAT=JSON.
...
## Evidence to capture
Primary signals: actual rows, rows examined, filtered percentage, index condition pushdown and handler_read counters.

Also correlate with performance_schema, sys schema, information_schema.innodb_trx, data_locks/data_lock_waits. Record database version and session-level settings because optimizer/memory behavior is version and configuration sensitive.
