package com.dnnthanh.marketplace.be.authorization.outbox.publisher;
...
/**
 * Publishes durable authorization-change events independently from interactive authorization HTTP
 * traffic.
 */
@Adapter
...
public class AuthorizationOutboxPublisher {
    ...
    /**
     * Publishes a bounded pending batch; rows are marked processed only after broker
     * acknowledgement.
     */
    ...
    public void publishBatch() {
        List<Map<String, Object>> rows =
                jdbc.sql(
                                """
                SELECT id, event_id, aggregate_id, event_type, payload_json
                FROM outbox_event WHERE status='PENDING' ORDER BY id LIMIT 100
                """)
                        .query()
                        .listOfRows();
        for (Map<String, Object> row : rows) {
            ...
            kafka.publish(
                            "AUDIT_EVENT".equals(event.eventType())
                                    ...
                                    : "marketplace.authorization.events",
                            aggregateId,
                            event)
                    .whenComplete(
                            (result, error) -> {
