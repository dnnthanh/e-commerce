package com.dnnthanh.marketplace.be.inventory.outbox.publisher;
...
/**
 * Independently scalable inventory outbox publisher. Business transactions only persist durable
...
public class InventoryOutboxPublisher {
    ...
    public void publishBatch() {
        ...
        for (Map<String, Object> row : rows) {
            ...
            kafka.publish("marketplace.inventory.events", aggregateId, event)
                    .whenComplete(
                            (result, error) -> {
                                if (error == null) {
                                    jdbc.sql(
