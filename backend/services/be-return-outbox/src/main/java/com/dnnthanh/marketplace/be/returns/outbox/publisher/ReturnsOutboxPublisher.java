package com.dnnthanh.marketplace.be.returns.outbox.publisher;
...
/**
 * Independently scalable returns outbox publisher. Business transactions only persist durable rows;
 * this process owns Kafka network I/O.
 */
@Adapter
@RequiredArgsConstructor
public class ReturnsOutboxPublisher {
    ...
    public void publishBatch() {
        List<Map<String, Object>> rows =
                jdbc.sql(
                                """
                SELECT id,event_id,aggregate_id,event_type,payload_json::text payload_json FROM outbox_event WHERE status='PENDING' ORDER BY id LIMIT 100
                """)
                        .query()
                        .listOfRows();
        for (Map<String, Object> row : rows) {
            ...
            kafka.publish("marketplace.return.events", aggregateId, event)
                    .whenComplete(
                            (result, error) -> {
                                if (error == null) {
                                    jdbc.sql(
