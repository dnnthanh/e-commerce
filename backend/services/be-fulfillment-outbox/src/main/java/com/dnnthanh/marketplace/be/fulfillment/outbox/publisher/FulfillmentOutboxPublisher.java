package com.dnnthanh.marketplace.be.fulfillment.outbox.publisher;
...
/**
 * Independently scalable fulfillment outbox publisher. Business transactions only persist durable
...
public class FulfillmentOutboxPublisher {
    ...
    public void publishBatch() {
        List<Map<String, Object>> rows =
                jdbc.sql(
                                """
                SELECT TOP 100 id,event_id,aggregate_id,event_type,payload_json FROM outbox_event WHERE status='PENDING' ORDER BY id
                """)
                        .query()
                        .listOfRows();
        for (Map<String, Object> row : rows) {
            ...
            kafka.publish("marketplace.fulfillment.events", aggregateId, event)
                    .whenComplete(
                            (result, error) -> {
                                if (error == null) {
                                    jdbc.sql(
                                                    """
                            UPDATE outbox_event SET status='PROCESSED',processed_at=SYSDATETIME() WHERE id=:id AND status='PENDING'
                            """)
                                            .param("id", id)
                                            .update();
                                }
