package com.dnnthanh.marketplace.be.review.outbox.publisher;
...
/**
 * Independently scalable review outbox publisher. Business transactions only persist durable rows;
...
public class ReviewOutboxPublisher {
    ...
    public void publishBatch() {
        ...
        for (Map<String, Object> row : rows) {
            ...
            kafka.publish("marketplace.review.events", aggregateId, event)
                    .whenComplete(
                            (result, error) -> {
                                if (error == null) {
                                    jdbc.sql(
