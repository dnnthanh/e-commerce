package com.dnnthanh.marketplace.be.media.outbox.publisher;
...
/**
 * Independently scalable media outbox publisher. Business transactions only persist durable rows;
...
public class MediaOutboxPublisher {
    ...
    public void publishBatch() {
        ...
        for (Map<String, Object> row : rows) {
            ...
            kafka.publish("marketplace.media.events", aggregateId, event)
                    .whenComplete(
                            (result, error) -> {
                                if (error == null) {
                                    jdbc.sql(
