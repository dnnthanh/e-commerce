package com.dnnthanh.marketplace.be.catalog.outbox.publisher;
...
/**
 * Independently scalable catalog outbox publisher. Business transactions only persist durable rows;
...
public class CatalogOutboxPublisher {
    ...
    public void publishBatch() {
        ...
        for (Map<String, Object> row : rows) {
            ...
            kafka.publish("marketplace.catalog.events", aggregateId, event)
                    .whenComplete(
                            (result, error) -> {
                                if (error == null) {
                                    jdbc.sql(
